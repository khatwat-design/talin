...
function SuccessContent() {
  ...
  useEffect(() => {
    if (firedPurchaseEvent.current || typeof window === "undefined") return;
    const raw = window.localStorage.getItem("talin-last-order");
    if (!raw) return;
    try {
      const parsed = JSON.parse(raw) as { total?: number; items?: Array<{ id: string; name?: string; price?: number; quantity?: number }> };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 rounded-3xl border border-[var(--color-border)] bg-white p-10 text-center shadow-[var(--shadow-soft)]">
      ...
      <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
        {whatsappNumber ? (
          <a href={`https://wa.me/${whatsappNumber.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(`مرحباً، أريد الاستفسار عن طلبي${invoice ? ` رقم ${invoice}` : ""}`)}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-6 py-3 text-sm font-bold text-white shadow-lg transition hover:brightness-110">
